...
public class BiHashMap<K,V> implements Map<K,V> {
	...
	public V put(K key, V value) {
		...
		HashSet<K> keys = (HashSet<K>) valueToKeys.get(value);
		if (keys == null) {
			keys = new HashSet();
		...
		if(rv != null && !rv.equals(value)) {
			HashSet<K> temp = valueToKeys.get(rv);
			temp.remove(key);
		}
	...
	public void putAll(Map<? extends K, ? extends V> m) {
		...
		for(Entry<? extends K,? extends V> temp : m.entrySet()) {
	...
	public HashSet<K> getKeys(Object value) {
		return (HashSet<K>) valueToKeys.get(value);
	}
	 ...
	 */
	@Override
	public Set<K> keySet() { return (Set<K>) keyToValues.keySet(); }
	...
	public static void main(String[] args) {
		...
		for (String x : (HashSet<String>) map.getKeys("ENGR202")) {
