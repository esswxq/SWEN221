...
public class Trick implements Serializable{
	...
	public void play(Player p, Card c) throws IllegalMove {
		...
		//if(!p.getHand().contains(c))        throw new IllegalMove("Card not exist!");			
		if(p.direction != getNextToPlay())  throw new IllegalMove("Not your turn!");		
		
		if(p.getHand().contains(c)) {
			if(cards[0] != null) {
				Card.Suit leaderSuit = cards[0].getSuit();	
				for(Card card : p.getHand()) {
					if(card.getSuit() == leaderSuit) {
						hasSameSuit = true;
					}
				}
				if(hasSameSuit) {
					if(c.getSuit() != leaderSuit) {
						throw new IllegalMove("Suit not match!");
					}
				}	
			}
		}
			else throw new  IllegalMove("Card not exist!");
